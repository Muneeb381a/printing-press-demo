...
const CategoryCombobox = forwardRef(({
  ...
  // Merge parent ref with internal inputRef so parent can call .focus()
  ...
  const handleKeyDown = (e) => {
    if (!open && (e.key === 'ArrowDown' || e.key === 'Enter')) {
      e.preventDefault(); setOpen(true); setQuery(''); return;
    }
    ...
    // Tab: close dropdown, let browser move focus naturally
    ...
    <div className={cn('flex flex-col gap-1', wrapperClassName)} ref={containerRef}>
      ...
      >
        ...
        <input
          ...
          readOnly={!open}
          className={cn(
            'flex-1 ps-8 pe-8 py-3 text-sm bg-transparent outline-none',
            open ? 'cursor-text placeholder-slate-400' : 'cursor-pointer',
            selected && !open ? 'font-semibold text-slate-900' : 'text-slate-900 placeholder-slate-400',
          )}
        />
            ...
            <ul ref={listRef} className="max-h-52 overflow-y-auto py-1">
              {filtered.length === 0 ? (
                <li className="px-4 py-3 text-sm text-slate-400 text-center">
                  {query ? `"${query}" nahi mila` : 'Koi product nahi'}
                </li>
              ) : (
                filtered.map((o, idx) => (
                  <li
                  ...
                  >
                    <span className={cn(
                      'flex-1 truncate',
                      idx === highlighted ? 'font-semibold text-slate-900' : 'font-medium text-slate-700',
                    )}>
                      {o.label}
                    </span>
